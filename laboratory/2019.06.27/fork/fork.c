...
int main() {
  ...
  // Create a connection queue, ignore child exit details
  ...
  while (1) {
    ...
    // Fork to create a process for this client and perform a test to
    // see whether we're the parent or the child.
    ...
    if (fork() == 0) {
      ...
      printf(
				"Connection established from %s, local port: %d, remote port: %d.\n",
				inet_ntoa(client_address.sin_addr), SERVER_PORT,
      ...
      do {
        ...
        if (strcmp(input, quit) != 0) {
          /* End input string. */
          input[length] = '\0';
          printf("From client %s:%d: %s", inet_ntoa(client_address.sin_addr),
                 (int)ntohs(client_address.sin_port), input);
          // Allocate output.
          char output[65535];
          int i;
